...
export async function Inspections({ address, t }: Props) {
  ...
  return (
    <section className="w-full lg:w-[720px] flex flex-col rounded-2xl overflow-hidden bg-gray-200 gap-3 p-5">
      <h3 className="font-bold text-black md:text-xl">{t("inspections")}</h3>

      {inspections.length === 0 ? (
        <div>
          <p className="text-center mt-5">{t("noInspections")}</p>
        </div>
      ) : (
        <>
          {inspections.map((item, index) => (
            <InspectionItem key={index} t={t} inspection={item} />
          ))}
        </>
      )}
    </section>
